# tazdan — App Store & Google Play Listing

## App Name
tazdan

## Subtitle (iOS, max 30 chars)
Crypto & Fiat Exchange

## Category
Finance

---

## Short Description (Google Play, max 80 chars)
Buy, sell & send crypto and fiat currencies — fast, secure, MENA-first.

---

## Full Description (max ~4,000 chars)

**tazdan** is the all-in-one financial super-app built for the MENA region. Whether you want to buy Bitcoin, send money to family, or earn on your digital assets — tazdan puts everything in one place.

**Buy & Sell Crypto**
Trade Bitcoin, Ethereum, USDT, Solana, XRP and more with real-time prices powered by live market data. Instant buys via card or bank transfer. No hidden spreads.

**Multi-Currency Fiat Wallet**
Hold USD, EUR, GBP, AED, SAR, EGP and LYD in your account. Deposit by bank wire, convert instantly, and withdraw back to your bank.

**P2P Marketplace**
Trade directly with other users at the rate you set. Our escrow system holds funds until both parties confirm — so every trade is safe.

**Virtual Visa Card**
Get a virtual Visa card in seconds, loaded directly from your tazdan wallet. Use it for online payments, subscriptions, and international purchases worldwide.

**International Transfers**
Send money across borders at competitive exchange rates. Track every transfer in real time.

**Referral Rewards**
Invite friends and earn when they trade. No limits on how much you can earn.

**Bank-grade Security**
• 2-Factor Authentication (TOTP / authenticator app)
• Face ID / Touch ID biometric login
• Passkey support (FIDO2)
• End-to-end encrypted communications
• KYC identity verification powered by Sumsub

---

## Keywords (iOS, comma-separated, max 100 chars total)
crypto,bitcoin,exchange,fiat,USDT,MENA,P2P,wallet,send money,visa card

---

## What's New (first release)
First public release of tazdan. Buy, sell, transfer and earn with crypto and fiat currencies — all in one app.

---

## Screenshots Needed (per device size)

### iPhone 6.9" (iPhone 16 Pro Max) — required
1. Home dashboard — balance cards + quick actions
2. Markets screen — live price list
3. P2P trade screen — order book + active listings
4. Virtual Visa card screen
5. Send/Receive flow — QR + address entry

### iPhone 6.7" (iPhone 15 Plus) — required
Same 5 screens at 1290 × 2796 resolution.

### iPad Pro 13" — optional but recommended
Same 5 screens at 2064 × 2752.

### Android Phone (Google Play) — required
Same 5 screens, 1080 × 1920 minimum (9:16).

### Feature Graphic (Google Play) — required
1024 × 500 px — use brand gradient (deep navy #030818 → violet #7B5CF0) with logo + tagline.

---

## Privacy Policy URL
https://tazdan.com/legal/privacy

## Support URL
https://tazdan.com/support

## Marketing URL
https://tazdan.com

---

## Age Rating
4+ (iOS) / Everyone (Android)
No objectionable content; financial app with identity verification.

## App Store Connect Info
- Bundle ID: com.tazdan.app
- SKU: tazdan-001
- Primary language: English (US)
- Territories: Algeria, Bahrain, Egypt, Iraq, Jordan, Kuwait, Lebanon, Libya, Morocco, Oman, Qatar, Saudi Arabia, Tunisia, United Arab Emirates, United Kingdom, United States, and global

## Google Play Info
- Package: com.tazdan.app
- Default language: en-US
- Countries: same as above + global
