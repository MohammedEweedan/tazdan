...
/**
 * Admin Ramps — on-ramp and off-ramp transaction browser.
 */
...
export default function AdminRamps() {
  ...
  const q = useQuery({
    queryKey: ['admin-ramps', tab, page],
    queryFn: () => tab === 'ONRAMP' ? adminService.rawOnRamps({ page, limit: 30 }) : adminService.rawOffRamps({ page, limit: 30 }),
  ...
  return (
    <View style={{ flex: 1, backgroundColor: p.bg }}>
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        <StackHeader title="Ramps" right={<><Text style={{ color: p.fgMuted, fontSize: 13, fontWeight: '700' }}>{q.data?.total ?? 0}</Text></>} />

        <View style={{ flexDirection: 'row', gap: 8, paddingHorizontal: 16, marginBottom: 10 }}>
          {(['ONRAMP', 'OFFRAMP'] as Tab[]).map((t) => {
            const on = tab === t;
            return (
        ...
        {q.isLoading ? <LoadingPulse fullscreen icon="arrow-up-circle-outline" label="Loading ramps…" /> : (
