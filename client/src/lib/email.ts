...
export async function sendMail(to: string, subject: string, html: string) {
  ...
  await transporter.sendMail({ from: `"Fortuni" <${SMTP_FROM}>`, to, subject, html });
}
...
export function waitlistConfirmation(email: string) {
  const html = base(
    ...
    <p class="muted">Thanks for joining the Fortuni early-access waitlist. You'll be among the first to know when we open your region — and you'll get <strong class="main">0% fees for your first 6 months</strong>.</p>
...
export function contactConfirmation(name: string, email: string) {
  const html = base(
    'We received your message — Fortuni',
